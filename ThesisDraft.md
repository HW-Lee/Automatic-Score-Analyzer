...
#### Staff Removal Performance Evaluation

Staff removal is an important stage because it might affect final recognition performance. Consequently, there are lots of studies only focused on improving the quality of staff removal. However, there is no formal quantitive method that evaluates how well the staves are removed. Dalitz et al. [8] introduce a way evaluating the robustness of a staff removal algorithm. Firstly, they gather a set of "ideal" images, which have perfectly horizontal and constant-width lines and their corresponding images after staff removal. Due to the cost building ground truths from real images and the difficulties of ground truths annotation, deformation is suggested. The deformation is a procedure that makes the ideal images more noisy, and Dalitz et al. list these deformation types:
...
Through testing the algorithm with these deformed images, an objective and acceptable quantitive method is needed. Dalitz et al. also introduce different metrics in terms of different observations.

- Pixel Level

	The most intuitive approach is counting the amount of misclassified pixels, and then divided by the amount of all black pixels.
	
	![](./ThesisDraft-src/pixel-level.png)
	
- Segmentation Region Level

	It considers the perspectiveness of human vision, so it counts error if missing, splitting, or merging [11] occurs.
	
	![](./ThesisDraft-src/segment-level.png)
	
- Staffline Interruptions

	It is similar to segmentation region level evaluation, but it considers the interruption of stafflines. Firstly, all interruptions of stafflines are obtained as a skeleton information. Subsequently, the evaluation system computes the error in the way similar to segmentation region level evaluation. It counts the error if there exists an interruption region that overlapped with not exactly one region.
	
	![](./ThesisDraft-src/interrupt-level.png)
	![](./ThesisDraft-src/interrupt-level2.png)

#### Music Symbols Segmentation

In a score, music symbols are constructed by some basic components, such as lines, curves, ellips, dots, etc. As the description above, staff removal aims at deleting interference during symbols detection and recognition, but it unavoidably suffers from breaking some symbols into fragments and the difficulty is raised. Randriamahefa et al. [2] propose attributed graph representation that polygonalizes and skeletonizes symbols from a set of points to a set of straight lines and junction points. In this way, symbols are less sensitive to the printed variance then the recognition can be expectedly better, it is a good decision to apply to the images after staff removal. On the other hand, Reed & Parker [3], Bellini et al. [4] do segmentation with stafflines. Bellini et al. design a hierarchical steps to complete tasks. After staff indication, the original image is divided into many smaller images that contain exactly one staff. For each small image, the first layer operation is to tell the regions without any symbol from those with symbols, such image is divided into regions where each region is annotated as either 1, with symbols, or 0, without any symbol. Similarly, Bellini et al. use different criteria in the case of different symbols, and the segmentation task can be done after three levels of the system.
...
After segmentation, the image has been divided into a set of bounding boxes where each box contain a symbol to be recognized. In this stage, the proposed methods are various depending on how features have been extracted. Randriamahefa et al. [2] characterize symbols as arrangement of lines, which is called skeletonization and polygonalization, and then use them as the features to be recognized. They just set some rules based on heuristic concepts. Reed & Parker [3] use the techniques that are similar than those suggested by Randriamahefa et al., they introduce line adjacency graph (LAG) for compressing a set of points into a set of connected points that can be formed as a graph. In addition, they process some special symbols such as accidentals, rests, and clefs with character profile, which simply computes the perpendicular distance of a character's contour from some reference axis, usually the peripheral boundaries of the bounding box. For note head recognition, they use template matching.

### 2-4. Contextual Reconstruction

When the recognition has been done, it is followed by contextual reconstruction, which is the first stage that organizes symbol information into a well-structured representation. Reed & Parker [3] suggest to represent the relationship between symbols as a graph. Under such representation, it provides a simple and intuitive way to transform the results to some computer-readable formats such as MIDI.

![](./ThesisDraft-src/contextual-graph-representation.png)
...
[11] M. Thulke, V. Margnet, A. Dengel, *A general approach to quality evaluation of document segmentation results*. Lecture Notes in ComputerScience, volume 1655, pp. 43-57, Springer (1999)