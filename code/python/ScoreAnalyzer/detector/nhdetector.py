...
def get_feat(img, width, info, tighten=False):
    ...
    if tighten: img = _binimg_tighten(img=img)
    img = misc.imresize(img, [width, width]) / 255.

    for s in range(width):
        feat += [0]
        for i, j in _get_positions(s):
            feat[-1] += img[i, j]
    for s in range(width-1):
        feat += [0]
        for i, j in _get_positions(width-2-s):
            feat[-1] += img.T[i, j]
...
class NoteHeadDetector(object):
    def __init__(self, info, model="notehead20160315-1", decision_thresh=0):
    ...
    def is_notehead(self, img, tighten=False):
        ...
        feat = get_feat(img=img, width=self.featwidth, info=self.info, tighten=tighten)
